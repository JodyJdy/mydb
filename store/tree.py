...
class BTree:
    ...
    def search(self,key):
        ...
        return node
    def _search_for_delete(self,key,node)->Node:
        while not isinstance(node,LeafNode):
            branch_node = node
            i = 0
            while i < (len(branch_node.keys)):
                #如果允许重复插入，新节点，插入左侧节点
                if key < branch_node.keys[i]:
                    node = branch_node.values[i]
                    break
                elif key == branch_node.keys[i]:
                    if  i + 1 < len(branch_node.values):
    ...
    def delete(self,key):
        node = self._search_for_delete(key, self.tree)
